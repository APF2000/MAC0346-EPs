...
function SIMULATOR.run(scenario_input)
  ...
  while count <= fight_number do
    ...
    local acerto = combate.acerto(unit_1, unit_2, weapons, math.random(1, 100), math.random(1, 100)) --flag de acerto do ataque
    if acerto == 1 then--ataque acertou
      local critical = combate.critical(unit_1, unit_2, weapons, math.random(1, 100)) --flag para indicar se ataque é crítico
    end
    scenario_input.units[unit_2].hp = combate.attack(unit_1, unit_2, weapons, critical) --atualiza vida do defensor
